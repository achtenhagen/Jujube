...
this.data = [];
var myTag = "";
function SearchAssistant(periodA, periodB, count) {
	timeRangeA = periodA;
	timeRangeB = periodB;
	displayCount = count;
}
...
SearchAssistant.prototype.setup = function() {
		...
		this.controller.get('headLine').update('Searching for songs...');
		...
		this.controller.setupWidget("playList",{
    		itemTemplate:"search/playlist", 
     		swipeToDelete:false, 
			autoconfirmDelete: false,
       		renderLimit: 100,
        	reorderable:false
        	},  this.playModel = {items: this.listItems});
		this.options = [];
...
SearchAssistant.prototype.loadMoreItems = function(){
	blockLimit++;
	this.setupdata();
};

SearchAssistant.prototype.databaseSuccess = function(database){
	try {
		this.excludeList = [];
...
SearchAssistant.prototype.databaseFailure = function(result){};

SearchAssistant.prototype.itemRemoved = function(event){
	this.data.splice(event.index, 1)
	this.controller.get('headLine').update('Found ' + this.data.length + " possible matches...");
};

SearchAssistant.prototype.optionTapped = function(event){
	if(event.index == 0)
	{
...
SearchAssistant.prototype.close =  function(e){};
SearchAssistant.prototype.open =  function(e){this.scrim.show();};
...
SearchAssistant.prototype.resetScroll = function(event){
...
SearchAssistant.prototype.tapped = function(event) {
...
SearchAssistant.prototype.showDialogBox = function(title,message){
	this.controller.showAlertDialog({
		onChoose: function(value) {},
		title:title,
		message:message,
...
SearchAssistant.prototype.setupdata = function() {
	var url = "";
	this.cookie = new Mojo.Model.Cookie('jujube_user')
	var cookieSettings = this.cookie.get();
	var exclude = cookieSettings.exclude
	if(exclude == true)
	{
		if(this.excludeList != null && this.excludeList.length > 0)
	{
		if(blockLimit > 0)
			url = "http://fioware.com/jujube/period_search.php?rangea=" + timeRangeA + "&rangeb=" + timeRangeB + "&limit=" + displayCount + "&exclude=[" + this.excludeList + "]&block=" + blockLimit
		else
			url = "http://fioware.com/jujube/period_search.php?rangea=" + timeRangeA + "&rangeb=" + timeRangeB + "&limit=" + displayCount + "&exclude=[" + this.excludeList + "]"
	}
	else{
		if(blockLimit > 0)
			url = "http://fioware.com/jujube/period_search.php?rangea=" + timeRangeA + "&rangeb=" + timeRangeB + "&limit=" + displayCount + "&block=" + blockLimit
		else
			url = "http://fioware.com/jujube/period_search.php?rangea=" + timeRangeA + "&rangeb=" + timeRangeB + "&limit=" + displayCount
	}
	}
	else
	{
...
SearchAssistant.prototype.checkSuccess = function(transport){
	try {
...
SearchAssistant.prototype.checkFailure = function(response){
